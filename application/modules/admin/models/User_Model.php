...
class  User_Model extends MY_Model
{
    ...
    var $column_order_export = array(null,null,'US.photo','US.fullname','US.username','RO.name','US.mytoken','US.email','US.last_logged_in','US.last_logged_out','US.status'); 
    var $column_search_export = array('US.photo','US.fullname','US.username','RO.name','US.mytoken','US.email','US.last_logged_in','US.last_logged_out','US.status');
    var $column_order = array(null,null,'US.photo','US.fullname','US.username','RO.name','US.mytoken','US.email','US.last_logged_in','US.last_logged_out','US.status'); 
    var $column_search = array('US.photo','US.fullname','US.username','RO.name','US.mytoken','US.email','US.last_logged_in','US.last_logged_out','US.status');
    ...
    private function _get_datatables_query()
    {
        if(!empty($_POST['export'])){
            $this->db->select('US.id,US.uuid,US.photo,US.fullname,US.username,RO.name AS role_name,US.mytoken,US.email,US.last_logged_in,US.last_logged_out,US.status');
        }else {
            $this->db->select('US.id,US.uuid,US.photo,US.fullname,US.username,RO.name AS role_name,US.mytoken,US.email,US.last_logged_in,US.last_logged_out,US.status');
        }
        $this->db->from('users AS US');
        $this->db->join('roles AS  RO', 'RO.id = US.role_id', 'left');
        if($this->session->userdata('role_id') != 1){
            $this->db->where_not_in('US.role_id' , 1);
        }
    ...
    function check_email(){           
        if($this->input->post('id')){
            $this->db->where_not_in('uuid', $this->input->post('id'));
        }
