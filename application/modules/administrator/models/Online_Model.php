...
class  Online_Model extends MY_Model
{
    ...
    var $column_order_export = array(null,null,'US.photo','US.fullname','ZX.name','US.email','US.last_logged_in','US.last_logged_out',); 
    var $column_search_export = array('US.photo','US.fullname','ZX.name','US.email','US.last_logged_in','US.last_logged_out',);
    var $column_order = array(null,null,'US.photo','US.fullname','ZX.name','US.email','US.is_login','US.last_logged_in','US.last_logged_out'); 
    var $column_search = array('US.photo','US.fullname','ZX.name','US.email','US.is_login','US.last_logged_in','US.last_logged_out');
    ...
    private function _get_datatables_query()
    {
        if(!empty($_POST['export'])){
            $this->db->select('US.id,US.uuid,US.photo,US.fullname,ZX.name AS role_name,US.email,US.last_logged_in,US.last_logged_out');
        }else {
            $this->db->select('US.id,US.uuid,US.photo,US.fullname,ZX.name AS role_name,US.email,US.is_login,US.last_logged_in,US.last_logged_out');
        }
        $this->db->from('users AS US');
        $this->db->join('roles AS  ZX', 'ZX.id = US.role_id', 'left');
        ...
        if($this->session->userdata('___is_login') != ''){
            $this->db->where('US.is_login', $this->session->userdata('___is_login'));
        }
        
        if ($this->session->userdata('___last_logged_in_start') != '' ||  $this->session->userdata('___last_logged_in_end') != '') {
            $start = date('Y-m-d', strtotime($this->session->userdata('___last_logged_in_start')));
            $end = date('Y-m-d', strtotime($this->session->userdata('___last_logged_in_end')));

            if ($start == $end) {
                $this->db->like('US.last_logged_in', date('Y-m-d', strtotime($start)));
            } else {
                $this->db->where('US.last_logged_in >= ', $start . ' 00:00:00');
                $this->db->where('US.last_logged_in <', $end . ' 23:59:59');
            }
        }
        if ($this->session->userdata('___last_logged_out_start') != '' ||  $this->session->userdata('___last_logged_out_end') != '') {
            $start = date('Y-m-d', strtotime($this->session->userdata('___last_logged_out_start')));
            $end = date('Y-m-d', strtotime($this->session->userdata('___last_logged_out_end')));

            if ($start == $end) {
                $this->db->like('US.last_logged_out', date('Y-m-d', strtotime($start)));
            } else {
                $this->db->where('US.last_logged_out >= ', $start . ' 00:00:00');
                $this->db->where('US.last_logged_out <', $end . ' 23:59:59');
            }
        }
        $i = 0;
        if(!empty($_POST['export'])){
