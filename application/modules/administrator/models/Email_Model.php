...
class  Email_Model extends MY_Model
{
    ...
    var $column_order_export = array(null,null,'US.photo','US.fullname','MJ.name','US.email','US.email_reset_at','US.email_reset_by','US.status'); 
    var $column_search_export = array('US.photo','US.fullname','MJ.name','US.email','US.email_reset_at','US.email_reset_by','US.status');
    var $column_order = array(null,null,'US.photo','US.fullname','MJ.name','US.email','US.email_reset_at','US.email_reset_by','US.status'); 
    var $column_search = array('US.photo','US.fullname','MJ.name','US.email','US.email_reset_at','US.email_reset_by','US.status');
    ...
    private function _get_datatables_query()
    {
        if(!empty($_POST['export'])){
            $this->db->select('US.id,US.uuid,US.photo,US.fullname,MJ.name AS role_name,US.email,US.email_reset_at,US.email_reset_by,US.status');
        }else {
            $this->db->select('US.id,US.uuid,US.photo,US.fullname,MJ.name AS role_name,US.email,US.email_reset_at,US.email_reset_by,US.status');
        }
        $this->db->from('users AS US');
        $this->db->join('roles AS  MJ', 'MJ.id = US.role_id', 'left');
        ...
        if ($this->session->userdata('___email_reset_at_start') != '' ||  $this->session->userdata('___email_reset_at_end') != '') {
            $start = date('Y-m-d', strtotime($this->session->userdata('___email_reset_at_start')));
            $end = date('Y-m-d', strtotime($this->session->userdata('___email_reset_at_end')));

            if ($start == $end) {
                $this->db->like('US.email_reset_at', date('Y-m-d', strtotime($start)));
            } else {
                $this->db->where('US.email_reset_at >= ', $start . ' 00:00:00');
                $this->db->where('US.email_reset_at <', $end . ' 23:59:59');
            }
        }
        if($this->session->userdata('___email_reset_by') != ''){
            $this->db->where('US.email_reset_by', $this->session->userdata('___email_reset_by'));
        }
    ...
    public function get_filter_by_email_reset_by() //3
    {
        $this->db->select('email_reset_by');
        $this->db->like('email_reset_by',  $this->input->get('term'), 'both');
        $this->db->order_by('email_reset_by', 'asc');
        $this->db->limit(10);
        $query =  $this->db->get('users')->result();
        $data = array();
        foreach ($query as $obj) {
            $data[] = [
                'label' => $obj->email_reset_by,
            ];
        }
        return $data;
    }
