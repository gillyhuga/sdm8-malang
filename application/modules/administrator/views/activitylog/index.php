...
<div class="page-button-data" id="lombok" data-url="administrator/activitylog/">
    <input type="hidden" id="trash" name="trash" value="<?= $this->session->userdata('___recycle_bin');?>">
    <?php if(is_admin()){?>
        <button id="button_export" data-url="administrator/activitylog/export" data-export="1" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_export">
    ...
    <?php if (has_permission(DELETE, 'administrator', 'module')) { ?>
        <button id="button_bulkdestroy" data-url="administrator/activitylog/bulkdestroy" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-danger waves-effect waves-classic button_bulkdestroy">
    ...
    <button id="button_refresh" data-url="administrator/activitylog/index" type="button" class="mb-2 btn btn-icon btn-outline-info waves-effect waves-classic button_refresh">
        <i class="icon md-refresh" aria-hidden="true"></i>
    </button>
    <div class="page-title" style="float: right;">ACTIVITYLOG</div>
</div>
...
<script>
    $(document).ready(function() {
        var show_activitylog = $('#show_all').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: true,
            // responsive: true,
            ...
            ajax: {
                url: "<?= base_url('administrator/activitylog/data_json') ?>",
                type: "POST",
                data: function(data) {}
            },
            columnDefs: [{
        ...
        socket.onmessage = function(e) {
            if(e.data == 'administrator_activitylog'){
                show_activitylog.ajax.reload(null, false);
            }
        };
        // --------------------------------------------------------------------------------------------------------[Websocket stop]
    });
