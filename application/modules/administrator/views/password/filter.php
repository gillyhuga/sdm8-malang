...
<form action="" id="form_filter">
    ...
    <div class="row mt-2">
        ...
        <div class="col-6 form-group">
            <small class="btn btn-xs btn-icon btn-default waves-effect waves-classic"">Password Reset At</small>
            <input type="text" class="form-control showCalRanges" id="___password_reset_at" name="___password_reset_at" placeholder="Password Reset At">
        </div>
        <div class="col-6 form-group">
            <small class="btn btn-xs btn-icon btn-default waves-effect waves-classic"">Password Reset By</small>
            <input type="hidden" id="_password_reset_by" value="<?= $this->session->userdata('___password_reset_by')?>">
            <input type="text" class="form-control" id="___password_reset_by" name="___password_reset_by" placeholder="Password Reset By">
        </div>
    </div>
...
<script>
    ...
    $('#___fullname').autocomplete({
        source: 'administrator/password/get_filter_by_fullname',
        autoFocus:true,
        minLength: 2,
    ...
    $('#___username').autocomplete({
        source: 'administrator/password/get_filter_by_username',
        autoFocus:true,
        minLength: 2,
        select: function(event, ui){
    ...
    $.ajax({
        url: "<?= base_url('administrator/password/edit_filter_by_role_id') ?>",
        dataType: 'JSON',
        type : 'POST',
        data: {
            _role_id : _role_id
    ...
    $("#___role_id").select2({
        ajax: {
            url: "<?= base_url('administrator/password/get_filter_by_role_id') ?>",
            dataType: 'JSON',
            type: "GET",
            delay: 250,
    ...
    $('#___email').autocomplete({
        source: 'administrator/password/get_filter_by_email',
        autoFocus:true,
        minLength: 2,
        select: function(event, ui){
    ...
    var _password_reset_by = $('#_password_reset_by').val();
    $('#___password_reset_by').val(_password_reset_by);
    $('#___password_reset_by').autocomplete({
        source: 'administrator/password/get_filter_by_password_reset_by',
        autoFocus:true,
        minLength: 2,
        select: function(event, ui){
            $('[name="password_reset_by"]').val(ui.item.label);
        }
    });
</script>