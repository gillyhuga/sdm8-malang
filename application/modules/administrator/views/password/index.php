...
<div class="page-button-data" id="lombok" data-url="administrator/password/">
    <input type="hidden" id="trash" name="trash" value="<?= $this->session->userdata('___recycle_bin');?>">
    <?php if(is_admin()){?>
        <button id="button_export" data-url="administrator/password/export" data-export="1" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_export">
    ...
    <?php if (has_permission(EDIT, 'administrator', 'password')) { ?>
        <button id="button_status" data-url="administrator/password/status" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_status">
    ...
    <div class="page-title" style="float: right;">PASSWORD</div>
</div>
        ...
        <table class="table table-hover dataTable table-striped w-full nowrap" data-plugin="dataTable" id="show_all">
            <thead>
                <tr>
                    ...
                    <th>Role </th>
                    <th>Email</th>
                    <th>Password Reset At</th>
                    <th>Password Reset By</th>
                    <th>Status</th>
                    <th>Action</th>
                </tr>
            </thead>
...
<script>
    $(document).ready(function() {
        var show_password = $('#show_all').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: true,
            // responsive: true,
            ...
            ajax: {
                url: "<?= base_url('administrator/password/data_json') ?>",
                type: "POST",
                data: function(data) {}
            },
            columnDefs: [{
        ...
        socket.onmessage = function(e) {
            if(e.data == 'administrator_password'){
                show_password.ajax.reload(null, false);
            }
        };
        // --------------------------------------------------------------------------------------------------------[Websocket stop]
    });
