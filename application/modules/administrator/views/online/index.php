...
<div class="page-button-data" id="lombok" data-url="administrator/online/">
    <input type="hidden" id="trash" name="trash" value="<?= $this->session->userdata('___recycle_bin');?>">
    <?php if(is_admin()){?>
        <button id="button_export" data-url="administrator/online/export" data-export="1" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_export">
    ...
    <button id="button_refresh" data-url="administrator/online/index" type="button" class="mb-2 btn btn-icon btn-outline-info waves-effect waves-classic button_refresh">
        <i class="icon md-refresh" aria-hidden="true"></i>
    </button>
    <div class="page-title" style="float: right;">ONLINE</div>
</div>
...
<script>
    $(document).ready(function() {
        var show_online = $('#show_all').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: true,
            // responsive: true,
            ...
            ajax: {
                url: "<?= base_url('administrator/online/data_json') ?>",
                type: "POST",
                data: function(data) {}
            },
            columnDefs: [{
        ...
        socket.onmessage = function(e) {
            if(e.data == 'administrator_online'){
                show_online.ajax.reload(null, false);
            }
        };
        // --------------------------------------------------------------------------------------------------------[Websocket stop]
    });
