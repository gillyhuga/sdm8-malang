...
<form action="" id="form_filter">
    ...
    <div class="row mt-2">
        ...
        <div class="col-6 form-group">
            <small class="btn btn-xs btn-icon btn-default waves-effect waves-classic"">Token Reset At</small>
            <input type="text" class="form-control showCalRanges" id="___token_reset_at" name="___token_reset_at" placeholder="Token Reset At">
        </div>
        <div class="col-6 form-group">
            <small class="btn btn-xs btn-icon btn-default waves-effect waves-classic"">Token Reset By</small>
            <input type="hidden" id="_token_reset_by" value="<?= $this->session->userdata('___token_reset_by')?>">
            <input type="text" class="form-control" id="___token_reset_by" name="___token_reset_by" placeholder="Token Reset By">
        </div>
    </div>
...
<script>
    ...
    $('#___fullname').autocomplete({
        source: 'administrator/token/get_filter_by_fullname',
        autoFocus:true,
        minLength: 1,
    ...
    $('#___username').autocomplete({
        source: 'administrator/token/get_filter_by_username',
        autoFocus:true,
        minLength: 1,
        select: function(event, ui){
    ...
    $.ajax({
        url: "<?= base_url('administrator/token/edit_filter_by_role_id') ?>",
        dataType: 'JSON',
        type : 'POST',
        data: {
            _role_id : _role_id
    ...
    $("#___role_id").select2({
        ajax: {
            url: "<?= base_url('administrator/token/get_filter_by_role_id') ?>",
            dataType: 'JSON',
            type: "GET",
            delay: 250,
    ...
    $('#___email').autocomplete({
        source: 'administrator/token/get_filter_by_email',
        autoFocus:true,
        minLength: 1,
        select: function(event, ui){
    ...
    var _token_reset_by = $('#_token_reset_by').val();
    $('#___token_reset_by').val(_token_reset_by);
    $('#___token_reset_by').autocomplete({
        source: 'administrator/token/get_filter_by_token_reset_by',
        autoFocus:true,
        minLength: 1,
        select: function(event, ui){
            $('[name="token_reset_by"]').val(ui.item.label);
        }
    });
</script>