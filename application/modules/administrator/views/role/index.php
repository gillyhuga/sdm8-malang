...
<div class="page-button-data" id="lombok" data-url="administrator/role/">
<input type="hidden" id="trash" name="trash" value="<?= $this->session->userdata('___recycle_bin');?>">
    <?php if (has_permission(ADD, 'administrator', 'role')) { ?>
        <button id="button_create" data-url="administrator/role/create" type="button" class="mb-2 btn btn-icon btn-outline-primary waves-effect waves-classic button_create">
            <i class="icon md-plus" aria-hidden="true"></i> <span class="hidden-sm-down">Add New</span>
        </button>
    <?php } ?>
    <?php if(is_admin()){?>
        <button id="button_import" data-url="administrator/role/import" type="button" class="mb-2 btn btn-icon btn-outline-warning waves-effect waves-classic button_import">
            <i class="icon md-download" aria-hidden="true"></i> <span class="hidden-sm-down">Import</span>
        </button>
    <?php } ?>
    <?php if(is_admin()){?>
        <button id="button_export" data-url="administrator/role/export" data-export="1" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_export">
    ...
    <?php if (has_permission(EDIT, 'administrator', 'role')) { ?>
        <button id="button_status" data-url="administrator/role/status" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_status">
    ...
    <?php if (has_permission(DELETE, 'administrator', 'role')) { ?>
        <button id="button_bulkdestroy" data-url="administrator/role/bulkdestroy" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-danger waves-effect waves-classic button_bulkdestroy">
            <i class="icon md-delete" aria-hidden="true"></i> <span class="hidden-sm-down">Bulk Delete</span>
        </button>
    <?php } ?>
    <?php if(is_superadmin()){?>
    <button id="button_bulkrestore" data-url="administrator/role/bulkrestore" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-warning waves-effect waves-classic button_bulkrestore">
    ...
    <button id="button_refresh" data-url="administrator/role/index" type="button" class="mb-2 btn btn-icon btn-outline-info waves-effect waves-classic button_refresh">
        <i class="icon md-refresh" aria-hidden="true"></i>
    </button>
    <div class="page-title" style="float: right;">ROLE</div>
</div>
...
<script>
    $(document).ready(function() {
        var show_role = $('#show_all').DataTable({
            ...
            ajax: {
                url: "<?= base_url('administrator/role/data_json') ?>",
                type: "POST",
                data: function(data) {}
            },
            columnDefs: [{
        ...
        socket.onmessage = function(e) {
            if(e.data == 'administrator_role'){
                show_role.ajax.reload(null, false);
            }
        };
        // --------------------------------------------------------------------------------------------------------[Websocket stop]
    });
