...
<div class="page-button-data" id="lombok" data-url="administrator/email/">
    ...
    <?php if(is_admin()){?>
        <button id="button_export" data-url="administrator/email/export" data-export="1" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_export">
    ...
    <button id="button_refresh" data-url="administrator/email/index" type="button" class="mb-2 btn btn-icon btn-outline-info waves-effect waves-classic button_refresh">
        <i class="icon md-refresh" aria-hidden="true"></i>
    </button>
    <div class="page-title" style="float: right;">EMAIL</div>
</div>
        ...
        <table class="table table-hover dataTable table-striped w-full nowrap" data-plugin="dataTable" id="show_all">
            <thead>
                <tr>
                    ...
                    <th>Role </th>
                    <th>Email</th>
                    <th>Email Reset At</th>
                    <th>Email Reset By</th>
                    <th>Status</th>
                    <th>Action</th>
                </tr>
            </thead>
...
<script>
    $(document).ready(function() {
        var show_email = $('#show_all').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: true,
            // responsive: true,
            ...
            ajax: {
                url: "<?= base_url('administrator/email/data_json') ?>",
                type: "POST",
                data: function(data) {}
            },
            columnDefs: [{
        ...
        socket.onmessage = function(e) {
            if(e.data == 'administrator_email'){
                show_email.ajax.reload(null, false);
            }
        };
        // --------------------------------------------------------------------------------------------------------[Websocket stop]
    });
