...
<form action="" id="form_filter">
    ...
    <div class="row mt-2">
        ...
        <div class="col-6 form-group">
            <small class="btn btn-xs btn-icon btn-default waves-effect waves-classic"">Email Reset At</small>
            <input type="text" class="form-control showCalRanges" id="___email_reset_at" name="___email_reset_at" placeholder="Email Reset At">
        </div>
        <div class="col-6 form-group">
            <small class="btn btn-xs btn-icon btn-default waves-effect waves-classic"">Email Reset By</small>
            <input type="hidden" id="_email_reset_by" value="<?= $this->session->userdata('___email_reset_by')?>">
            <input type="text" class="form-control" id="___email_reset_by" name="___email_reset_by" placeholder="Email Reset By">
        </div>
    </div>
...
<script>
    ...
    $('#___fullname').autocomplete({
        source: 'administrator/email/get_filter_by_fullname',
        autoFocus:true,
        minLength: 1,
    ...
    $('#___username').autocomplete({
        source: 'administrator/email/get_filter_by_username',
        autoFocus:true,
        minLength: 1,
        select: function(event, ui){
    ...
    $.ajax({
        url: "<?= base_url('administrator/email/edit_filter_by_role_id') ?>",
        dataType: 'JSON',
        type : 'POST',
        data: {
            _role_id : _role_id
    ...
    $("#___role_id").select2({
        ajax: {
            url: "<?= base_url('administrator/email/get_filter_by_role_id') ?>",
            dataType: 'JSON',
            type: "GET",
            delay: 250,
    ...
    $('#___email').autocomplete({
        source: 'administrator/email/get_filter_by_email',
        autoFocus:true,
        minLength: 1,
        select: function(event, ui){
    ...
    var _email_reset_by = $('#_email_reset_by').val();
    $('#___email_reset_by').val(_email_reset_by);
    $('#___email_reset_by').autocomplete({
        source: 'administrator/email/get_filter_by_email_reset_by',
        autoFocus:true,
        minLength: 1,
        select: function(event, ui){
            $('[name="email_reset_by"]').val(ui.item.label);
        }
    });
</script>