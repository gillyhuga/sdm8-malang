...
<div class="page-button-data" id="lombok" data-url="administrator/operation/">
    ...
    <?php if (has_permission(ADD, 'administrator', 'operation')) { ?>
        <button id="button_create" data-url="administrator/operation/create" type="button" class="mb-2 btn btn-icon btn-outline-primary waves-effect waves-classic button_create">
            <i class="icon md-plus" aria-hidden="true"></i> <span class="hidden-sm-down">Add New</span>
        </button>
    <?php } ?>
    <?php if(is_admin()){?>
        <button id="button_import" data-url="administrator/operation/import" type="button" class="mb-2 btn btn-icon btn-outline-warning waves-effect waves-classic button_import">
            <i class="icon md-download" aria-hidden="true"></i> <span class="hidden-sm-down">Import</span>
        </button>
    <?php } ?>
    <?php if(is_admin()){?>
        <button id="button_export" data-url="administrator/operation/export" data-export="1" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_export">
            <i class="icon md-file-text" aria-hidden="true"></i> <span class="hidden-sm-down">Export</span>
        </button>
    <?php } ?>
    <?php if (has_permission(EDIT, 'administrator', 'operation')) { ?>
        <button id="button_status" data-url="administrator/operation/status" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_status">
            <i class="icon md-check" aria-hidden="true"></i> <span class="hidden-sm-down">Change Status</span>
        </button>
    <?php } ?>
    <?php if (has_permission(DELETE, 'administrator', 'operation')) { ?>
        <button id="button_bulkdestroy" data-url="administrator/operation/bulkdestroy" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-danger waves-effect waves-classic button_bulkdestroy">
            <i class="icon md-delete" aria-hidden="true"></i> <span class="hidden-sm-down">Bulk Delete</span>
        </button>
    <?php } ?>
    <?php if(is_admin()){?>
    <button id="button_bulkrestore" data-url="administrator/operation/bulkrestore" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-warning waves-effect waves-classic button_bulkrestore">
    ...
    <button id="button_refresh" data-url="administrator/operation/index" type="button" class="mb-2 btn btn-icon btn-outline-info waves-effect waves-classic button_refresh">
...
<div class="panel">
    <div class="panel-body">
        <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable" id="show_all">
            <thead>
                <tr>
                    <th style="width: 2%;">
...
<script>
    $(document).ready(function() {
        var show_operation = $('#show_all').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: true,
            responsive: true,
        ...
        socket.onmessage = function(e) {
            if(e.data == 'administrator_operation'){
                show_operation.ajax.reload(null, false);
            }
        };
        // --------------------------------------------------------------------------------------------------------[Websocket stop]
    });
