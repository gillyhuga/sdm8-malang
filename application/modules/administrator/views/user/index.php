...
<div class="page-button-data" id="lombok" data-url="administrator/user/">
    <input type="hidden" id="trash" name="trash" value="<?= $this->session->userdata('___recycle_bin');?>">
    <?php if(is_admin()){?>
        <button id="button_export" data-url="administrator/user/export" data-export="1" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_export">
    ...
    <?php if (has_permission(EDIT, 'administrator', 'user')) { ?>
        <button id="button_access" data-url="administrator/user/status" data-status="1" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_status">
            <i class="icon md-check" aria-hidden="true"></i> <span class="hidden-sm-down">Allow Access</span>
        </button>
        <button id="button_access" data-url="administrator/user/status" data-status="0" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-danger waves-effect waves-classic button_status">
    ...
    <button id="button_refresh" data-url="administrator/user/index" type="button" class="mb-2 btn btn-icon btn-outline-info waves-effect waves-classic button_refresh">
...
<script>
    $(document).ready(function() {
        var show_user = $('#show_all').DataTable({
            ...
            ajax: {
                url: "<?= base_url('administrator/user/data_json') ?>",
                type: "POST",
                data: function(data) {}
            },
            columnDefs: [{
