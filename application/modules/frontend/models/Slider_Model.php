...
class  Slider_Model extends MY_Model
{
    ...
    var $column_order = array(null,null,'slider','title','subtitle','order_menu','note','status'); 
    var $column_search = array('slider','title','subtitle','order_menu','note','status');
    ...
    {
        $this->db->select('id,slider,title,subtitle,order_menu,note,status');
        $this->db->from('fr_slider');
    ...
    function duplicate_check_order_menu(){           
        ...
        $this->db->where('is_deleted', 0);
        return $this->db->get('fr_slider')->num_rows();            
    }
