...
<div class="page-button-data" id="lombok" data-url="kesiswaan/agenda/">
<input type="hidden" id="trash" name="trash" value="<?= $this->session->userdata('___recycle_bin');?>">
    <?php if (has_permission(ADD, 'kesiswaan', 'agenda')) { ?>
        <button id="button_create" data-url="kesiswaan/agenda/create" type="button" class="mb-2 btn btn-icon btn-outline-primary waves-effect waves-classic button_create">
            <i class="icon md-plus" aria-hidden="true"></i> <span class="hidden-sm-down">Add New</span>
        </button>
    <?php } ?>
    <?php if (has_permission(EDIT, 'kesiswaan', 'agenda')) { ?>
        <button id="button_status" data-url="kesiswaan/agenda/status" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-success waves-effect waves-classic button_status">
            <i class="icon md-check" aria-hidden="true"></i> <span class="hidden-sm-down">Change Status</span>
        </button>
    <?php } ?>
    <?php if (has_permission(DELETE, 'kesiswaan', 'agenda')) { ?>
        <button id="button_bulkdestroy" data-url="kesiswaan/agenda/bulkdestroy" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-danger waves-effect waves-classic button_bulkdestroy">
            <i class="icon md-delete" aria-hidden="true"></i> <span class="hidden-sm-down">Bulk Delete</span>
        </button>
    <?php } ?>
    <?php if(is_superadmin()){?>
    <button id="button_bulkrestore" data-url="kesiswaan/agenda/bulkrestore" style="display: none;" type="button" class="mb-2 btn btn-icon btn-outline-warning waves-effect waves-classic button_bulkrestore">
    ...
    <?php } ?>
    <button id="button_refresh" data-url="kesiswaan/agenda/index" type="button" class="mb-2 btn btn-icon btn-outline-info waves-effect waves-classic button_refresh">
        <i class="icon md-refresh" aria-hidden="true"></i>
    </button>
    <div class="page-title" style="float: right;">AGENDA</div>
</div>
...
<script>
    $(document).ready(function() {
        var show_agenda = $('#show_all').DataTable({
            processing: true,
            serverSide: true,
            autoWidth: true,
            // responsive: true,
            ...
            ajax: {
                url: "<?= base_url('kesiswaan/agenda/data_json') ?>",
                type: "POST",
                data: function(data) {},
            },
            columnDefs: [{
