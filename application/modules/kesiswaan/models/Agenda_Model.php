...
class  Agenda_Model extends MY_Model
{
    ...
    var $column_order = array(null,null,'FR.foto','FR.title','QG.nama','FR.tanggal_mulai','FR.note','FR.status'); 
    var $column_search = array('FR.foto','FR.title','QG.nama','FR.tanggal_mulai','FR.note','FR.status');
    ...
    {
        $this->db->select('FR.id,FR.foto,FR.title,QG.nama AS kategori,FR.tanggal_mulai,FR.note,FR.status');
        $this->db->from('fr_agenda AS FR');
        $this->db->join('fr_kategori_berita AS  QG', 'QG.id = FR.kategori', 'left');
